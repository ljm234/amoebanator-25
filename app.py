...
@st.cache_data
def load_val_preds(path: Path) -> tuple[Any, Any]:
    if path.exists():
        try:
            df = pd.read_csv(path)
...
def net_benefit(y_true: np.ndarray, prob: np.ndarray, t: float) -> tuple[Any, ...]:
...
def sweep_best_threshold(y_true: np.ndarray, prob: np.ndarray, lo: float = 0.05, hi: float = 0.30, step: float = 0.01) -> dict[str, Any] | None:
...
def conformal_metrics(y_true: np.ndarray, p_high: np.ndarray, qhat: float) -> dict[str, Any] | None:
...
def charts_block(df: pd.DataFrame) -> None:
...
def ood_panel(df: pd.DataFrame) -> None:
    st.subheader("Out-of-Distribution check (Phase 3)")
    if df.empty:
        st.info("No cases yet.")
        return
    last = {str(k): v for k, v in df.iloc[-1].to_dict().items()}
    try:
        s = ood_score(last)
...
def main() -> None:
    # Import UI panels lazily so non-UI utilities can be tested without Streamlit.
    from ml.ui_live_patient import render_live_patient_panel
